...
function chanceCommunityChest() {
	...
	if (p.position === 2 || p.position === 17 || p.position === 33) {
		...
		popup("<img src='images/community_chest_icon.png' style='height: 50px; width: 53px; float: left; margin: 8px 8px 8px 0px;' /><div style='font-weight: bold; font-size: 16px; '>Mining Pool:</div><div style='text-align: justify;'>" + communityChestCards[communityChestIndex].text + "</div>", function() {
...
function streetrepairs(houseprice, hotelprice) {
	...
	if (cost > 0) {
		...
		// If function was called by Mining Pool.
		if (houseprice === 40) {
			addAlert(p.name + " lost $" + cost + " to Mining Pool.");
		} else {
...
function useJailCard() {
	...
	if (p.communityChestJailCard) {
		...
		// Insert the get out of Magic City free card back into the Mining Pool deck.
...
window.onload = function() {
	...
	// Shuffle Chance and Mining Pool decks.
